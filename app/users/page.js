...
export default function Page() {
  ...
  useEffect(() => {
    async function getUsers() {
      try {
        const res = await fetch('https://backend-beta-bay.vercel.app/users');
        if (!res.ok) {
        ...
        <tbody>
          {items.map((item) => (
            <tr key={item.id}>
              ...
              <td><button href={`/users/del/${item.id}`} className="btn btn-danger" type="button" onClick={() => handleDelete(item.id)}>Del</button></td>
            </tr>
          ))}
        </tbody>
      </table>
